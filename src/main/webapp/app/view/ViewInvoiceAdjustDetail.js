Ext.define(App.path('view.ViewInvoiceAdjustDetail'), {
	extend : 'Ext.form.Panel',
	border : false,
	itemId : 'viewinvoiceadjustdetail',
	xtype : 'viewinvoiceadjustdetail',
	id : 'viewinvoiceadjustdetail',
		...
		items : [{
			...
			items : [{
				...
				columns : [ 
				            ...
				            {header : 'Điều chỉnh cho HĐ', resizable: false, dataIndex : 'parent_invoice_no', editor : {}},
				            {header : 'Loại điều chỉnh', resizable: false, dataIndex : 'adjust_type', editor : {}},
	...
	activate : function() {
		...
		store.removeAll();
		this.file_url = null;
		this.down('#dataloadaddadjust').setDisabled(true);
	},
	search: function(params){
		...
		store.getProxy().extraParams.adjust_type = adjust_type;
        								...
        								App.Action.FilePreview(App.Session.userName, response.result.url, function(options,success, response) {
        									if (success) {
        										...
        										if (response.success) {
        											...
        											me.down('#dataloadaddadjust').setDisabled(false);
        										} else {
	...
	UpdateDB : function() {
		...
		App.Action.InvoiceAdjust(App.Session.userName, me.file_url, me.adjust, function(options,success, response) {
			Ext.get(document.body).unmask();
			if (success) {
				response = Ext.decode(response.responseText);
				if (response.success) {
				...
				me.file_url = null;
				me.down('#dataloadaddadjust').setDisabled(true);
			} else {
				Ext.MessageBox.alert('Thông báo', 'Thêm dữ liệu thất bại. '+ response.info);
			}
		});
